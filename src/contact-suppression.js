...
function effectiveWorkspaceStage(conversation, pipelineStages = []) {
  const customStage = String(conversation?.custom_attributes?.crm_stage || "").trim();
  if (
    (Array.isArray(pipelineStages) ? pipelineStages : [])
      .some((stage) => stage?.id === customStage)
  ) {
    return customStage;
  }

  const labels = access.labelNames(conversation).map((label) => String(label).toLowerCase());
...
function shouldClassifyAsChatwootResolved(conversation, pipelineStages = []) {
  return conversation?.status === "resolved" &&
    effectiveWorkspaceStage(conversation, pipelineStages) === "new";
}
...
function classifyWorkspaceConversation(
  ...
  if (shouldClassifyAsChatwootResolved(conversation, pipelineStages)) {
